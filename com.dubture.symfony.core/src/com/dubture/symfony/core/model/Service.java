...
/**
 * 
 * The Service class represents a Symfony2 Service retrievable from the
...
public class Service extends SourceType {
	...
	public ISourceModule getSourceModule() {
		if (className != null && namespace != null) {
			...
			IDLTKSearchScope scope = project != null ? SearchEngine
					.createSearchScope(project) : SearchEngine
					.createWorkspaceScope(PHPLanguageToolkit.getDefault());
			IType[] types = PhpModelAccess.getDefault().findTypes(namespace,
					className, MatchRule.EXACT, 0, 0, scope, null);
			if (types.length >= 1) {
